...
import bcrypt from "bcryptjs";
import crypto from "crypto";
import { OtpPurpose, OtpChannel, Role } from "@prisma/client";
...
export const verifyLoginOtp = async (req: Request, res: Response) => {
  try {
    const { otpId, code } = req.body as { otpId?: number; code?: string };
    ...
    if (!otp || !otp.user || otp.purpose !== OtpPurpose.LOGIN) {
      return res.status(400).json({ message: "Invalid OTP request." });
    }
    if (otp.consumedAt) return res.status(400).json({ message: "OTP already used." });
    ...
    const deviceId = (req.header("x-device-id") || "") || crypto.randomUUID?.() || "unknown";
...
export const changePassword = async (req: Request, res: Response) => {
  try {
    ...
    return res.json({
      message:
        "Password changed successfully. Please log in again with the new password.",
    });
  } catch (error) {
    console.error("Change password error:", error);
    return res.status(500).json({ message: "Internal server error." });
